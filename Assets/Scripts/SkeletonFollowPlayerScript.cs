...
public class SkeletonFollowPlayerScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        setAnimationBool("isForward");


        attackTimer = 0; // Start ready to attack if close enough
    }
    
    void setAnimationBool(string boolname)
    {
    ...
    void FaceDirection(Vector3 targetDirection)
    {
        float angle = Mathf.Atan2(targetDirection.y, targetDirection.x) * Mathf.Rad2Deg;
        float rAngle = angle360(angle);

        Debug.Log("Skeleton angle: " + rAngle);

        // angle from 137 to -134 is left
        // angle from -134 to -53 is down
        // angle from -53 to 34
        // angle from 34 to 137
        if (rAngle > 45 && rAngle <= 135)
        {
            Debug.Log("Facing Up");
            myAnimator.SetBool("isRight", false);
        }
        else if (rAngle > 135 && rAngle <= 225)
        {
            Debug.Log("Facing Left");
            myAnimator.SetBool("isRight", false);
            mySpriteRenderer.flipX = true;
        }
        else if (rAngle > 225 && rAngle <= 315)
        {
        ...
        myAnimator.SetFloat("angle", rAngle);
    }
